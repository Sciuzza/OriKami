...
public class SpecialTreePlat : MonoBehaviour
{
    ...
    private IEnumerator RotateFromAToB(Quaternion fromRot, Quaternion toRot, float timeTaken)
    {
        ...
        {
            timeTaken = InTime - ((((1 -this.timePassed) * this.fractionOut) / this.OutTime) * this.InTime);
        ...
        while (this.timePassed < 1)
        {
            ...
            /*
            foreach (var gb in this.ObjsToRotate)
            {
                var tempRot = gb.transform.rotation;
                tempRot.eulerAngles = new Vector3(gb.transform.rotation.eulerAngles.x, gb.transform.rotation.eulerAngles.y, this.gameObject.transform.rotation.eulerAngles.z);
               // gb.transform.rotation = Quaternion.Slerp(fromRot, toRot, this.timePassed);
                gb.transform.rotation = tempRot;
            }
            */
